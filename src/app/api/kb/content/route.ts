...
import { getContentBody } from '@/lib/vivo-api';

export async function POST(request: NextRequest) {
  try {
    const body = await request.json();
    const { token_id, content_ids, content_type } = body;
    ...
    const data = await getContentBody(content_ids || [], content_type || 'markdown', tokenInfo.token);
    ...
    console.error('KB content error:', error);
    return NextResponse.json(
